...
extern const unsigned int collectSong_sampleRate;
extern const unsigned int collectSong_length;
extern const signed char collectSong_data[];
# 12 "game.c" 2
# 1 "eatSong.h" 1


extern const unsigned int eatSong_sampleRate;
extern const unsigned int eatSong_length;
extern const signed char eatSong_data[];
# 13 "game.c" 2
...
void updatePlayer() {
    ...
    if((~((*(volatile unsigned short *)0x04000130)) & ((1<<6)))) {
        if ( collisionMap[((sandwich.worldRow - sandwich.rdel)*(256)+(sandwich.worldCol))]
                && collisionMap[((sandwich.worldRow - sandwich.rdel)*(256)+(sandwich.worldCol + sandwich.width - 1))] &&
                     sandwich.worldRow >= 0) {
                sandwich.worldRow -=sandwich.rdel;

            if ( vOff >= 1 && (sandwich.worldRow - vOff) <= (160/2)){
                vOff -= 1;
            }
        }
    }
    if((~((*(volatile unsigned short *)0x04000130)) & ((1<<7)))) {
       if (collisionMap[((sandwich.worldRow + sandwich.rdel + sandwich.height-1)*(256)+(sandwich.worldCol))]
                && collisionMap[((sandwich.worldRow + sandwich.height + sandwich.rdel - 1)*(256)+(sandwich.worldCol + sandwich.width - 1))]
                && (sandwich.worldRow + sandwich.height ) <= 256 ) {
                sandwich.worldRow +=sandwich.rdel;

            if (vOff <= (256 - 160) && (sandwich.worldRow - vOff) >= (160/2)) {
    ...
    if((~((*(volatile unsigned short *)0x04000130)) & ((1<<5)))) {
         if ( collisionMap[((sandwich.worldRow)*(256)+(sandwich.worldCol - sandwich.cdel))]
                && collisionMap[((sandwich.worldRow + sandwich.height - 1)*(256)+(sandwich.worldCol - sandwich.cdel))]
                && sandwich.worldCol >= 0 ) {
                sandwich.worldCol -=sandwich.cdel;

            if ( hOff >= 1 && (sandwich.worldCol - hOff) <= (240/2)) {
    ...
    if((~((*(volatile unsigned short *)0x04000130)) & ((1<<4)))) {
        if (collisionMap[((sandwich.worldRow)*(256)+(sandwich.worldCol + sandwich.width -1))]
                && collisionMap[((sandwich.worldRow + sandwich.height -1)*(256)+(sandwich.worldCol + sandwich.width -1))]
                && (sandwich.worldCol + sandwich.height ) <= 256) {
                sandwich.worldCol +=sandwich.cdel;

            if (hOff <= (256 - 240) && (sandwich.worldCol - hOff) >= (240/2)) {
    ...
    if((!(~(oldButtons)&((1<<1))) && (~buttons & ((1<<1)))) && num_portal ){
        num_portal = 0;
        int min = 100000;
        int new_min;
        int count = 0;
...
void drawPlayer() {
    if (sandwich.hide) {
        ...
        shadowOAM[0].attr0 = (0xFF & sandwich.worldRow - vOff) | (0<<14);
        shadowOAM[0].attr1 = (0x1FF & sandwich.worldCol - hOff) | (0<<14);
        shadowOAM[0].attr2 = ((0)<<12) | ((sandwich.curFrame)*32+(sandwich.aniState))| ((0)<<10);
    }
}
...
void drawCollectibles(){
    for(int i = 0; i < 3; i++){
        if (invisble[i].hide) {
            ...
            shadowOAM[i+1].attr0 = (0xFF & invisble[i].worldRow - vOff) | (0<<14);
            shadowOAM[i+1].attr1 = (0x1FF & invisble[i].worldCol - hOff) | (0<<14);
            shadowOAM[i+1].attr2 = ((0)<<12) | ((3 + invisble[i].curFrame)*32+(invisble[i].aniState + 2));
        }
    }
    if (speed.hide) {
            shadowOAM[5].attr0 |= (2<<8);
    } else {
        shadowOAM[5].attr0 = (0xFF & speed.worldRow - vOff) | (0<<14);
        shadowOAM[5].attr1 = (0x1FF & speed.worldCol - hOff) | (0<<14);
        shadowOAM[5].attr2 = ((0)<<12) | ((3 + speed.curFrame)*32+(speed.aniState + 1));
    }

    if (portal.hide) {
            shadowOAM[15].attr0 |= (2<<8);
    } else {
        shadowOAM[15].attr0 = (0xFF & portal.worldRow - vOff) | (0<<14);
        shadowOAM[15].attr1 = (0x1FF & portal.worldCol - hOff) | (0<<14);
        shadowOAM[15].attr2 = ((0)<<12) | ((3 + portal.curFrame)*32+(portal.aniState));
    }
...
void drawMeatballs(){
    for(int i = 0; i < 5; i++){
        if (meatBalls[i].hide) {
            ...
            shadowOAM[i+6].attr0 = (0xFF & meatBalls[i].worldRow - vOff) | (0<<14);
            shadowOAM[i+6].attr1 = (0x1FF & meatBalls[i].worldCol - hOff) | (0<<14);
            shadowOAM[i+6].attr2 = ((0)<<12) | ((3 + meatBalls[i].curFrame)*32+(meatBalls[i].aniState + 3));
        }
    }
}
...
void drawFrog(){
    if (frog.hide) {
        ...
        shadowOAM[12].attr0 = (0xFF & frog.worldRow - vOff) | (0<<14);
        shadowOAM[12].attr1 = (0x1FF & frog.worldCol - hOff) | (0<<14);
        shadowOAM[12].attr2 = ((0)<<12) | ((frog.curFrame + 6)*32+(frog.aniState));
    }
}
